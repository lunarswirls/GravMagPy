...
if [[ -z "${PRED_CSV}" || -z "${DIPOLE_CSV}" ]]; then
  ...
  output_dir="$(cd -- "$(dirname -- "${FIRST_CSV}")" && pwd)"
  ...
  if [[ -z "${PRED_CSV}" ]]; then PRED_CSV="${output_dir}/${base}_dipole_fit_predictions.csv"; fi
  if [[ -z "${DIPOLE_CSV}" ]]; then DIPOLE_CSV="${output_dir}/${base}_dipole_fit_dipoles.csv"; fi
fi
mkdir -p -- "$(dirname -- "${PRED_CSV}")" "$(dirname -- "${DIPOLE_CSV}")"
